...
An AstrBot image is sent as a `data:image/...;base64,...` value. AstrBot
`Record.convert_to_base64()` normalizes audio bytes to WAV, so audio is sent
as `audio_base64` with `audio_format: "wav"` regardless of source filename or
MIME type. Kokoro transcribes incoming audio before generating
