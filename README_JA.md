...
> 💡 **開発者に優しい仕様（100% 全自動ストレージ閉ループ・番犬）**：
> Rust/Tauri の頻繁なデバッグやテストでは、`target` ディレクトリが数十GBに達しディスクを圧迫しがちです。Kokoro Engine は**全ライフサイクル閉ループ・ストレージ番犬（Kokoro Storage Sentinel, KSS）**を内蔵：`npm install` で安全な Git フックを自動構成し、ブランチ切り替え時にマーカーを記録して、次の組み込み npm 開発コマンド実行時または手動 prune 時に孤立キャッシュを優先解消、`npm run tauri dev` や `npm run dev` の起動前フックで 12GB 警戒水位を自動維持（コンパイラロックの安全回避付き）、テスト成果物は 30 分の保護ウィンドウで自動収束、ドライブ空き容量 5GB 未満で緊急遮断。**開発起動前の自動チェック・トリミングにより、日常開発での手動クリーンアップは一切不要です**。
> いつでも `npm run storage:doctor` で透明なヘルスチェック看板を確認できます。詳細は [開発者向けストレージ全ライフサイクル管理＆高速ビルド（sccache）ガイド](docs/developer-storage-guide.md) をご覧ください。
