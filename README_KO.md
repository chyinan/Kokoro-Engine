...
- **Developer-friendly & Zero-waste**: Rust/Tauri 프로젝트에서 흔히 발생하는 `target` 디렉터리 수십 GB 비대화 및 디스크 부족 문제를 완벽히 해결. 브랜치 전환 시 고아 캐시 자동 정리, 테스트 바이너리 수명 주기 관리, 여유 공간 부족 시 긴급 보호가 포함된 완전 자동화 스토리지 센티넬(KSS) 내장.
...
> 💡 **개발자 친화적 안내 (100% 완전자동 폐쇄루프 스토리지 워치독)**:
> Rust/Tauri 개발 및 테스트 과정에서 `target` 디렉터리가 수십 GB로 급증하여 디스크가 부족해지기 쉽습니다. Kokoro Engine은 **전 수명주기 폐쇄루프 스토리지 워치독(Kokoro Storage Sentinel, KSS)**을 내장하고 있습니다: `npm install` 시 안전한 Git 훅 자동 구성, 브랜치 전환 시 마커를 기록하고 다음 내장 npm 개발 명령 실행 또는 수동 prune 시 고아 세션 우선 정리, `npm run tauri dev` 또는 `npm run dev` 실행 시 12GB 경계 수위 자동 유지(컴파일러 락 안전 양보 탑재), 테스트 생성물 30분 보호 윈도우 기반 자동 수렴, 드라이브 여유 공간 5GB 미만 시 긴급 보호 차단이 동작합니다. **개발 실행 전 자동 점검 및 정리로 일상적인 수동 정리 명령어가 전혀 필요하지 않습니다**.
> 언제든지 `npm run storage:doctor` 명령으로 투명한 진단 대시보드를 확인할 수 있으며, 자세한 내용은 [개발자 스토리지 전 수명주기 관리 및 초고속 빌드(sccache) 가이드](docs/developer-storage-guide.md)를 참고하세요.
...
- `Codex Runtime — Experimental`: 선택 가능한 로컬 런타임 브리지입니다. `codex app-server --listen stdio://`를 통해 사용자가 이미 설정한 Codex 인증, 모델 및 provider 구성을 재사용합니다. Kokoro는 Codex 자격 증명을 읽거나 저장하지 않으며 Codex 구성을 변경하지 않습니다. [Codex Runtime 문서](docs/codex-runtime.md)를 참고하세요.
...
- AstrBot 통합: 인증된 범용 Webhook으로 AstrBot 메시지를 Kokoro에 연결하고, 캐릭터 및 대화 단위의 텍스트·이미지·오디오를 처리하여 텍스트 또는 미디어로 응답합니다. [AstrBot 통합 가이드](integrations/astrbot-kokoro/README.md)를 참고하세요.
...
👉 [**Kokoro Engine 공식 QQ 그룹: 810398532**](https://qm.qq.com/q/V8ljISeAUM)
