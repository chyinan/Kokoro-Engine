...
> 💡 **Дружелюбность к разработчикам (Автоматическое управление хранилищем)**:
> Интенсивная разработка на Rust может быстро занять десятки гигабайт кэшем сборки. Kokoro Engine оснащен **полностью автоматическим замкнутым сторожем хранилища** (удерживает `target` в пределах 12 ГБ при разработке, вычищает устаревшие сессии при переключении веток Git и запускает экстренную очистку при свободном месте менее 5 ГБ).
> Выполните `npm run storage:doctor` для мгновенной диагностики. Подробнее в [Руководстве разработчика по хранилищу и сборке](docs/developer-storage-guide.md).
