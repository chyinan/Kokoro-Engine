...
- **Developer-friendly & Zero-waste**: Устраняет проблему бесконтрольного разрастания каталога `target` (до десятков гигабайт) и переполнения диска в проектах Rust/Tauri. Встроенный автономный сторож хранилища (KSS) автоматически очищает осиротевшие сессии при смене веток, утилизирует тестовые сборки и защищает диск при нехватке места.
...
> 💡 **Дружелюбность к разработчикам (100% автоматический замкнутый сторож хранилища)**:
> Частая отладка и тестирование проектов на Rust/Tauri могут быстро раздуть каталог `target` до десятков гигабайт и занять весь диск. В Kokoro Engine встроен **полноцикловый замкнутый сторож хранилища (Kokoro Storage Sentinel, KSS)**: `npm install` автоматически настраивает безопасные хуки Git, переключение веток лишь записывает маркер, а следующая встроенная npm-команда разработки или ручной prune выполняет приоритетную очистку сиротских кэшей, запуск `npm run tauri dev` или `npm run dev` автоматически удерживает уровень кэша в пределах 12 ГБ (с безопасным пропуском при активных блокировках компилятора), тестовые артефакты защищены 30-минутным окном, а свободное место на диске < 5 ГБ вызывает аварийный сброс. **Автоматическая проверка и очистка перед каждым запуском разработки избавляют от необходимости выполнять ручные команды очистки**.
> Выполните `npm run storage:doctor` в любое время для прозрачной диагностики. Подробнее в [Руководстве разработчика по полному жизненному циклу хранилища и быстрой сборке (sccache)](docs/developer-storage-guide.md).
...
- `Codex Runtime — Experimental`: необязательный локальный runtime-мост через `codex app-server --listen stdio://`, который повторно использует уже настроенные пользователем аутентификацию, модель и provider Codex. Kokoro не читает и не хранит учетные данные Codex и не изменяет конфигурацию Codex. См. [документацию Codex Runtime](docs/codex-runtime.md).
...
- Поддержка импорта карточек персонажей SillyTavern.

### Удалённое взаимодействие

- Встроенные сервисы QQ, Telegram, Discord, LINE и Webhook.
- Интеграция с AstrBot: подключает сообщения AstrBot к Kokoro через аутентифицированный универсальный webhook, поддерживает обработку текста, изображений и аудио с учетом персонажа и диалога, а также текстовые и медиа-ответы. См. [руководство по интеграции AstrBot](integrations/astrbot-kokoro/README.md).
- Поддерживаемые каждой платформой типы сообщений передаются в ИИ-конвейер.
...
👉 [**Официальная QQ-группа Kokoro Engine: 810398532**](https://qm.qq.com/q/V8ljISeAUM)
