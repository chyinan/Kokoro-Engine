...
> Частая отладка и тестирование проектов на Rust/Tauri могут быстро раздуть каталог `target` до десятков гигабайт и занять весь диск. В Kokoro Engine встроен **полноцикловый замкнутый сторож хранилища (Kokoro Storage Sentinel, KSS)**: `npm install` автоматически настраивает безопасные хуки Git, переключение веток мгновенно фиксируется с приоритетной очисткой сиротских кэшей при следующей сборке, запуск `npm run tauri dev` или `npm run dev` автоматически удерживает уровень кэша в пределах 12 ГБ (с безопасным пропуском при активных блокировках компилятора), тестовые артефакты защищены 30-минутным окном, а свободное место на диске < 5 ГБ вызывает аварийный сброс. **Автоматическая проверка и очистка перед каждым запуском разработки избавляют от необходимости выполнять ручные команды очистки**.
