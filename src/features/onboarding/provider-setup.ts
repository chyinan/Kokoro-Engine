...
function applySetupToConfig(config: Readonly<LlmConfig>, setup: Readonly<ProviderSetup>): LlmConfig {
...
/** Discovers models from the local Ollama API or an OpenAI-compatible endpoint. */
...
/** Persists the focused fields and returns the exact normalized config written. */
export async function saveProviderSetup(
    config: Readonly<LlmConfig>,
    setup: Readonly<ProviderSetup>,
    dependencies: ProviderSetupDependencies = {},
): Promise<LlmConfig> {
    const updated = applySetupToConfig(config, setup);
    const persist = dependencies.saveLlmConfig ?? saveLlmConfig;
    await persist(updated);
    return updated;
}

/** Runs a connection test against a normalized setup without persisting advanced settings. */
export async function testProviderSetup(
    ...
    return test(applySetupToConfig(config, setup));
}
